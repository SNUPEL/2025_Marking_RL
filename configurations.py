...
def get_configurations(args=None):
    parser = argparse.ArgumentParser(description="Attention based model for solving the Travelling Salesman Problem with Reinforcement Learning")
    ...
    if configs.bl_warmup_epochs is None:
        configs.bl_warmup_epochs = 1 if configs.baseline == 'rollout' else 0
    assert (configs.bl_warmup_epochs == 0) or (configs.baseline == 'rollout')
    assert configs.epoch_size % configs.batch_size == 0, "Epoch size must be integer multiple of batch size!"
    return configs